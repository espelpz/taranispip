...
if (params.st_profile) { ch_st_profile = file( params.st_profile, checkIfExists: true ) } //else { if (params.skip_get_profile) { ch_st_profile = Channel.value( 'False' ) } }
                                                                                          //else { if (params.skip_get_profile) { ch_st_profile = Channel.empty() } }
...
if (params.save_reference)           summary['Save Genome Indices'] = 'Yes' // --> se utiliza en uncompress de gff y tal
...
process CHECK_SAMPLESHEET {
    ...
    then
        awk -F, '{if(\$4 != "") {print \$0}}' $samplesheet > fasta_id.csv
    fi
...
def validate_input_fastq(LinkedHashMap sample) {
    ...
    def is_sra = sample.is_sra.toBoolean() // esto qué es?
    def is_ftp = sample.is_ftp.toBoolean() // esto qué es?
    def fastq_1 = sample.fastq_1
    def fastq_2 = sample.fastq_2
    def md5_1 = sample.md5_1 // esto qué es?
    def md5_2 = sample.md5_2 // esto qué es?
...
def validate_input_fasta(LinkedHashMap sample) {
    ...
    fasta_sample = file(fasta, checkIfExists: true)
...
/*
 * STEP 8: Analyze Schema with Taranis
 */
...
/*
 * STEP 10: Gene by gene analysis with Taranis
 */
...
// COMENTADO get_software_versions TEMPORALM.
...
// COMENTADO output_documentation TEMPORALM.
